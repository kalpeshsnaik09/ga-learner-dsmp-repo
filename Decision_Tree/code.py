...
plt.bar(fully_paid,fully_paid.index)
plt.show()
...
# Code starts here
X_train['int.rate']=X_train['int.rate'].map(lambda x:float(str(x).replace('%',''))/100)
X_test['int.rate']=X_test['int.rate'].map(lambda x:float(str(x).replace('%',''))/100)
